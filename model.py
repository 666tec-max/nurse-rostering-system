...
class NurseRosteringModel:
    def __init__(self, num_nurses, num_days, nurses_list, shift_requirements=None, shifts_config=None):
        """
    ...
    def add_constraints(self):
        ...
        # If no night shifts defined, skip
        if not self.night_shifts:
            return
        ...
        for n in range(self.num_nurses):
            ...
            is_night = []
            for d in range(self.num_days):
                # is_night[d] <=> sum(x[n,d,s] for s in night_shifts) == 1
                # Since max 1 shift per day, we can just sum them
            ...
            # If is_night[d], then cannot work Day shift on d+1
            for d in range(self.num_days - 1):
                # if is_night[d] => sum(x[n, d+1, s] for s in day_shifts) == 0
                self.model.Add(
            ...
            for d in range(self.num_days - 1):
                # Check if night block ends at d
                ...
                # If end_block AND consec_nights[d] >= 2 => day d+2 is OFF (No shifts at all)
        ...
        if self.shift_requirements:
            for d in range(self.num_days):
                for s in self.shifts:
                    ...
                    self.model.Add(
                        sum(self.x[(n, d, s)] for n in range(self.num_nurses)) >= req
                    )
                ...
                for d in range(self.num_days):
                    ...
                    for s in self.shifts:
                        if self.solver.Value(self.x[(n, d, s)]) == 1:
                            assigned = s
                            break
                    schedule[nurse_name].append(assigned if assigned else '-')
            return schedule
        return None
